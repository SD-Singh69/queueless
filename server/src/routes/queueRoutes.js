...
// Helper: Calculate average service duration in minutes
async function calculateAverageServiceTime(shopId) {
  ...
  if (!completedEntries.length) return 5;

  const totalDurationMs = completedEntries.reduce((acc, entry) => {
    return (
      acc +
      (new Date(entry.servedAt) - new Date(entry.createdAt))
    );
  }, 0);
...
// Helper: Send queue position alerts
async function processQueueAlerts(shopId) {
  ...
  for (let index = 0; index < waitingTokens.length; index++) {
    ...
    // Notify when 3rd in queue
    if (position === 3 && !entry.notifiedPosition3) {

      if (entry.customerPhone) {
        await sendSMSNotification(
          entry.customerPhone,
          "QueueLess Update: You are now #3 in line. Please prepare to reach the counter."
        );
      }
...
router.get("/status/:tokenId", async (req, res) => {
  ...
  try {
    ...
    const token = await QueueEntry.findById(
      req.params.tokenId
    );
